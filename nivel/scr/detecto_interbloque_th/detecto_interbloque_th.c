		...
		if (cantidad_interbloquedos(t_personaje.l_personajes,
			...
			if (param_nivel.Recovery) {
				...
				if (tipo == O_TO_N_MUERTE) {
					un_personaje_aux = busca_personaje_simbolo(buffer[0], t_personaje.l_personajes,
							&indice);
...
log_in_disk_niv(LOG_LEVEL_INFO,
		"marchar_personaje_c_recursos total de personajes %d", tot_personajes);
...
for (cont_personajes = 0; cont_personajes < tot_perosnajes; cont_personajes++) {
